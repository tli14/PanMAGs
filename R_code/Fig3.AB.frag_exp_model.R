...
write(equation_line, file = "R_code/Fig3.frag_incomp/frag_equation.txt", append=TRUE, sep = "\t")

## Create new file for model data.
model_line <- paste(c("Species", "xvalues","yvalues"), collapse = "\t")
write(model_line, file = "R_code/Fig3.frag_incomp/frag_model.txt", append=TRUE, sep = "\t")
...
for ( i in unique(frag_data$Species)){ 
  ...
  write.table(new_equation_line, file = "R_code/Fig3.frag_incomp/frag_equation.txt", 
  ...
  write.table(new_model_line, file = "R_code/Fig3.frag_incomp/frag_model.txt", 
              append=TRUE, sep = "\t", row.names = FALSE, col.names = FALSE, quote=FALSE)
}
