...
incomp_data <- read.table(file = "R_code/Fig2.frag_incomp/incomp_CG100_core.txt",  sep = '\t', header = TRUE)

## Create new file for equation (exponential model)
equation_line <- paste(c("Species", "a", "b", "Rsquared", "pvalue"), collapse = "\t")
write(equation_line, file = "R_code/Fig2.frag_incomp/incomp_equation.txt", append=TRUE, sep = "\t")

## Create new file for model data.
model_line <- paste(c("Species", "xvalues","yvalues"), collapse = "\t")
write(model_line, file = "R_code/Fig2.frag_incomp/incomp_model.txt", append=TRUE, sep = "\t")
...
for ( i in unique(incomp_data$Species)){ 
  ...
  write.table(new_equation_line, file = "R_code/Fig2.frag_incomp/incomp_equation.txt", append=TRUE, sep = "\t", row.names = FALSE, col.names = FALSE, quote=FALSE)
  xvalues <- seq(0, 5, 0.1)
  ...
  write.table(new_model_line, file = "R_code/Fig2.frag_incomp/incomp_model.txt", append=TRUE, sep = "\t", row.names = FALSE, col.names = FALSE, quote=FALSE)
}
