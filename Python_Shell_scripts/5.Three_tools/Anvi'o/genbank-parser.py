## Modified the original script: https://github.com/elizabethmcd/genomes-MAGs-database/blob/master/scripts/genbank-parser.py

## To generate a tab-delimited file to define external gene calls from Genbank files given by Prokka.
#!/usr/bin/env python
...
for record in SeqIO.parse(GBK, "genbank"):
    ...
    for f in record.features:
        if f.type == 'CDS':
            ...
            OUT_CDS.write('%d\t%s\t%d\t%d\t%s\t%s\t%s\t%s\t%s\n' %(gene_id, contig, beg, end, direction, partial, call_type, source, version))
